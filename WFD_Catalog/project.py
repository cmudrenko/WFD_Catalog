...
def newMeal():
    if request.method == 'POST':  # get data from the form
        newMeal = MealOption(name=request.form['name'], ingredients=request.form['ingredients'], days_id=request.form['days_id'], user_id=login_session['user_id'])
        session.add(newMeal)
        session.commit()
...
def showMeal(days_id, meal_id):
    ...
    meal.user_id != login_session['user_id']:
