...
# Home page to view all meals that are assigned and unassigned to days of the week
@app.route('/')
...
def newMeal():
    if request.method == 'POST':  # get data from the form
        newMeal = MealOption(name=request.form['name'],
...
def showMeal(days_id, meal_id):
    ...
    if 'username' not in login_session or meal.user_id != login_session['user_id']:
        ...
        return render_template('publicmeal.html', days=days,
                               meal=meal)
...
def editMeal(days_id, meal_id):
    ...
    if request.method == 'POST':
        # if name is empty it will be unchange from previous set selection
        if request.form['name'] == "":  
            editedMeal.name = editedMeal.name
        else:
            editedMeal.name = request.form['name']

        # if ingredients is empty it will return unchange from previous set selection
...
# disconnect from the login session disabling editing and deleting functionality
